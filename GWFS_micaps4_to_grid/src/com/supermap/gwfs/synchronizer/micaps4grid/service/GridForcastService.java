package com.supermap.gwfs.synchronizer.micaps4grid.service;

import java.util.Map;

import com.supermap.gcpp.core.common.UniObject;
import com.supermap.gwfs.synchronizer.micaps4grid.dao.Grid_forcastDao;
...
public class GridForcastService
{
	 ...
	 * @throws
	 */
	public UniObject findGridForcastValid(UniObject uniObject , String element_val,int origin_value)
	{
		return grid_forcastDao.findGridForcastValid(uniObject, element_val,origin_value);
	}
	/**
	 * 
	 * @Description: 查询服务器Id
	...
	public int savaGridForcast(UniObject uniObject ,String sequnce , String valid ,String level )
	{
		//时效
		...
		//层次
		Map<String, Map<String, String>> levelMap = (Map<String, Map<String,String>>) uniObject.getValue("level");
		for (String id : levelMap.keySet())
		{
			for (String val : levelMap.get(id).keySet())
			{
				if(val.equals(level))
				{
					uniObject.setIntegerValue("level_id", Integer.parseInt(id));
					uniObject.setStringValue("level_name", levelMap.get(id).get(val));
					uniObject.setIntegerValue("level_value", Integer.parseInt(val));
					break ;
				}
			}
		}
	...
	/**
	 * 
	 * @Description: 查找初始场下要素的层次信息
	 * @return UniObject
	 * @throws
	 */
	public UniObject findGridForcastLevel(UniObject uniObject, String element_val, int origin_id)
	{
		return grid_forcastDao.findGridForcastLevel(uniObject, element_val,origin_id);
	}
}
