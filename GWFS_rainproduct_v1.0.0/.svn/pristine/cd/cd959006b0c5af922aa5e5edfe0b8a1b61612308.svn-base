package com.supermap.gwfs.rainproduct.dao;
...
//				System.out.print(origin_id+ "\t" + origin_name+ "\t" + origin_value1 + "\t" + element_id+ "\t"+element_name  + "\t" +element_value1+ "\t"+forecast_level_id+ "\t"+forecast_level_name+ "\t"+forecast_level_value);
		...
		{
			logger.error("查询grid_forcast数据异常 ， 异常 : " + e);
		}
		finally
		{
...
//				System.out.print(origin_id+ "\t" + origin_name+ "\t" + origin_value1 + "\t" + element_id+ "\t"+element_name  + "\t" +element_value1+ "\t"+forecast_level_id+ "\t"+forecast_level_name+ "\t"+forecast_level_value);
		...
		{
			logger.error("查询grid_forcast的sequnce数据异常 ， 异常 : " + e);
		}
		finally
		{
			JDBCHelper.free(rs, smt, conn);
...
//				System.out.print(origin_id+ "\t" + origin_name+ "\t" + origin_value1 + "\t" + element_id+ "\t"+element_name  + "\t" +element_value1+ "\t"+forecast_level_id+ "\t"+forecast_level_name+ "\t"+forecast_level_value);
		...
		{
			logger.error("查询grid_forcast的valid数据异常 ， 异常 : " + e);
		}
		finally
		{
			JDBCHelper.free(rs, smt, conn);
	...
	public int saveGridForcast(UniObject uniObject)
	{
		...
		try
		{
			...
			smt.setDate(8, new java.sql.Date(simple.parse(uniObject.getStringValue("forcast_date")).getTime()));
			...
			smt.setInt(12, uniObject.getIntegerValue("forecast_level_id"));
			smt.setString(13, uniObject.getStringValue("forecast_level_name"));
			smt.setString(14, uniObject.getStringValue("forecast_level_value"));
			...
			smt.setString(22, df.format(new java.util.Date()));
			logger.info("executeUpdate \n" + smt.toString().substring(smt.toString().indexOf(":") + 1));
			result = smt.executeUpdate();
			logger.info("保存操作 ，受影响的行数 ： " + result);
		}
		catch (Exception e)
		{
			logger.error("保存grid_forcast数据异常 ， 异常 : " + e);
		}
		finally
		{
			JDBCHelper.free(rs, smt, conn);
	...
	public UniObject findGridForcastServer(UniObject uniObject)
	{
		...
		{
			logger.error("获取本机IP异常 , 异常 : " + e);
		}
		catch (SQLException e)
		{
			logger.error("查询serverId异常 , 异常 : " + e);
		}
		finally
		{
			JDBCHelper.free(rs, smt, conn);
